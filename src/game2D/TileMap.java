...
public class TileMap 
{
	...
	public Image getTileImage(int x, int y)
	{
		...
		if (ch == '.') return null; // Blank space
		 //Debugging: Print if a tile is accessed beyond valid bounds
	    if (x >= mapWidth) { 
	        System.out.println("⚠️ Attempted to draw beyond the last tile! Column: " + x);
	        return null;
	    }
	 ...
	 */
	public void draw(Graphics2D g, int xoff, int yoff, int screenWidth, int scrollX) {
		if (g == null) return;

	    Image img = null;
	    Rectangle clip = g.getClipBounds(); 
	    //Calculate the last valid column to prevent rendering extra tiles
	    int lastValidColumn = Math.min(mapWidth, (scrollX + screenWidth) / tileWidth);

	    for (int r = 0; r < mapHeight; r++) {
	    	for (int c = Math.max(0, (scrollX / tileWidth)); c < lastValidColumn; c++) {  // ✅ Limit rendering to last column
	            int xc = xoff + c * tileWidth;
	            int yc = yoff + r * tileHeight;

	            // ✅ Prevent drawing past visible screen
	            if (xc + tileWidth < clip.x || xc >= clip.x + clip.width) continue;
	            if (yc + tileHeight < clip.y || yc >= clip.y + clip.height) continue;

	            img = getTileImage(c, r);
	            
	            if (img != null) {
	                g.drawImage(img, xc, yc, null);
	                //System.out.println("Drawing tile at: " + c + "," + r + " -> Screen: " + xc + "," + yc);
	            }
	        }
	    }
	}
