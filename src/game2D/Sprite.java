...
public class Sprite {
	...
	*/
    ...
    {
    	 if (!render) return;
    	    int boxWidth = (int)(getImage().getWidth(null) * xscale);
    	    int boxHeight = (int)(getImage().getHeight(null) * yscale);
    	    g.drawRect((int)x + xoff, (int)y + yoff, boxWidth, boxHeight);
    }
	...
	*/
    public void drawBoundingCircle(Graphics2D g)
    {
    	if (!render) return;
    	 ...
    	 int diameter = (int)(Math.max(getImage().getWidth(null) * xscale, getImage().getHeight(null) * yscale));
    	    g.drawOval((int)x + xoff, (int)y + yoff, diameter, diameter);
    }
